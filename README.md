...
## Overview
There have been some cases where seemingly innocuous images posted on the Internet have led to grave consequences. [One such case](https://www.army.mil/article/75165/geotagging_poses_security_risks) involved the location of an aviation compound being revealed and then attacked by adversaries after soldiers posted images from inside on social media. The location of the compound was located due to the EXIF (Exchangeable Image File Format) data embedded in each photo: Camera information and settings, image information, latitude and longitude, and more. Fortunately, no human lives were lost, but this case serves as a lesson that security is essential in our increasingly digital world.
...
If you'd like to give the container a chosen name upon creation, you can add the `--name your-container-name` option to the above command.
...
6. Our container is running in "detached" mode (the `-d` in our `docker run` command) so we can interact with it while letting it do it's thing in the background. We want to enter the container to see if the scrubbed images were produced and saved. We will do so with `docker exec -ti your-container-name /bin/bash`. You should now be inside your container in the app's directory.
